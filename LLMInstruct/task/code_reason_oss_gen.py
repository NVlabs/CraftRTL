...
class CodeReasonOSSTask(BaseTask):
    ...
    def construct_prompt(self, example: dict):
        prompt = self.prompt_template.format(problem=example['problem'], reference_code=example['oss_input'])
        return prompt
    ...
    def parse(self, response_text: str) -> Optional[str]:
        ...
        if "<SOLUTION>" and "</SOLUTION>" in response_text:
            ...
            ed = response_text.rfind("</SOLUTION>")
            if st is not None and ed is not None:
                solution = response_text[st:ed].strip()
        elif "<SOLUTION>" in response_text:
            st = response_text.rfind("<SOLUTION>") + len("<SOLUTION>")
            if st is not None:
                solution = response_text[st:].strip()
        ...
        if "<REASON>" and "</REASON>" in response_text:
        ...
        elif "<REASON>" and "<SOLUTION>" in response_text:
            st = response_text.find("<REASON>") + len("<REASON>")
            ed = response_text.rfind("<SOLUTION>")
            if st is not None and ed is not None:
                reasoning = response_text[st:ed].strip()
            ...
            if ed is not None:
                reasoning = response_text[:ed].strip()
    ...
    def __call__(self, example: dict):
        ...
        result=parse_markdown_code_block(result) 
